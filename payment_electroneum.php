...
class plgJ2StorePayment_electroneum extends J2StorePaymentPlugin
{
	/**
	 * @var $_element  string  Should always correspond with the plugin's filename,
	 *                         forcing it to be unique
	 */
    var $_element    = 'payment_electroneum';
	...
	function __construct(& $subject, $config)
	{
		...
		JHtml::script(Juri::base().'plugins/j2store/payment_electroneum/src/electroneum.js'); 
		JHtml::stylesheet(Juri::base().'plugins/j2store/payment_electroneum/src/electroneum.css'); 
    ...
    function _prePayment( $data )
    {
		...
		$vars->apikey = $this->params->get('apikey');
		$vars->secret = $this->params->get('secret');
		$vars->outlet = $this->params->get('outlet');
		$vars->amount = $data['orderpayment_amount'];
		
		$currency_values= $this->getCurrency($data['order']);
		$vars->currencycode = $currency_values['currency_code'];
		$vars->order_id = $data['order_id'];
    ...
    function _process( $data )
    {
		...
		$json = array();
			 ...
			 {
				  $return['success'] = 0;
				  $json ['error'] = 'Unknown Error was found';
				  return $json;
			 }
